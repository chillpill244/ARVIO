...
class SeriesViewModel @Inject constructor(
    ...
    override suspend fun fetchCategoriesAndItems(): Pair<List<String>, Map<String, List<MediaItem>>> {
        val catalog = iptvRepository.getSeriesCatalog()
    ...
    suspend fun getSeriesDetailsWithTmdbId(item: MediaItem): MediaItem? {
        return try {
            ...
            val searchResults = tmdbApi.searchMulti(
                apiKey = com.arflix.tv.util.Constants.TMDB_API_KEY,
                query = item.title
            )
            ...
            if (tvShowResult != null && tvShowResult.id != 0) {
                ...
                val tvDetails = runCatching {
                    tmdbApi.getTvDetails(tvShowResult.id, com.arflix.tv.util.Constants.TMDB_API_KEY)
                }.getOrNull()
                ...
                if (tvDetails != null) {
                    ...
                    val enrichedItem = item.copy(
                        ...
                        image = tvDetails.posterPath?.let { com.arflix.tv.util.Constants.IMAGE_BASE + it } ?: "",
                        backdrop = tvDetails.backdropPath?.let { com.arflix.tv.util.Constants.BACKDROP_BASE_LARGE + it },
