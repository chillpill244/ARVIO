...
class StreamRepository @Inject constructor(
    ...
    private val streamCacheKey = stringPreferencesKey("stream_result_cache")
    ...
    private fun processStreams(streams: List<StremioStream>, addon: Addon): List<StreamSource> {
        ...
        return filtered
            .map { stream ->
                ...
                // Extract embedded subtitles from stream; skip any with blank URLs since
                // ExoPlayer cannot load a subtitle track with no source.
                val embeddedSubs = stream.subtitles?.mapIndexedNotNull { index, sub ->
                    val subUrl = sub.url?.trim().orEmpty()
                    if (subUrl.isBlank()) return@mapIndexedNotNull null
        ...
        subtitleAddons.flatMap { addon ->
            runCatching {
                withTimeout(SUBTITLE_TIMEOUT_MS) {
                    ...
                    val mapped = response.subtitles?.mapIndexedNotNull { index, sub ->
                        val subUrl = sub.url?.trim().orEmpty()
                        if (subUrl.isBlank()) return@mapIndexedNotNull null
                        val normalizedLang = normalizeLanguageCode(sub.lang)
                        val langFallback = sub.lang?.trim()?.lowercase()?.take(2) ?: "und"
